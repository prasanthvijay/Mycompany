...
import { Container, Content, InputGroup, Input, List,ListItem,Picker,Button,Icon,Form,Item,Spinner,Card, CardItem, Body } from 'native-base';
import styles from './styles';
const background = require('../images/shadow.jpg');
const logo = require('../images/Michelin.png');
const logonew = require('../images/logo.png');
...
async handleLayoutChange(event:Event){
		...
		if(width>height){
			...
			await this.setState({screenType: 'Landscape', height : height,width : width/1.1,imagewidth:width/5,imageheight:height/3.5});
			

		} else {

			await this.setState({screenType: 'Portrait', height : height,width : width/1.1,imagewidth:width/6.5 ,imageheight:height/3});
...
<Image
				...
				/> 	
			<Image
				style={{width: 220, height: 90, marginBottom:0   }}
				source={logo}
				/> 	
			</View>
