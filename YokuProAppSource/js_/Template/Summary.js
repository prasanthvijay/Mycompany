...
module.exports = class Resource extends Component {
 constructor(props) {
    super(props);
    this.state = {
   screenType: '',eventsDetails:[],isVisible:true,types:'Circle', color: "black", size: 50,isModalVisible: false,retry:false,resultData:false
...
async componentDidMount(){
		...
		this.summary(empid,companyid);
}
 summary(empid,companyid){
	...
	_this.setState({isVisible:true,retry:false});
		fetch(url+'manageappajax?type=readingHistoryList&empid='+empid+'&customerid='+companyid, {
		method: 'POST',
		headers: {
...
async handleLayoutChange(event:Event){
		...
		if(width>height){
			...
			await this.setState({screenType: 'Portrait', height : height,width : width,imagewidth:width/1.1 ,imageheight:height/2.4,width,headerimagewidth:width/3 ,headerimageheight:height/18,paddingBody:width/7,modalBody:width/2});
		}
 	...
 	var microAppList = _this.state.eventsDetails.map((eventsDetails, index) => {
            return (
				  ...
				  <Thumbnail source={logo} />
                                <Body>
                                    <Text note>Category Name : {eventsDetails.catageryname}</Text>
                                    <Text note>Session Name : {eventsDetails.sessionname}({eventsDetails.keyword})</Text>
                                </Body>
                            </Left> 							
                          </CardItem>
                         
                          <CardItem >
                              <Text>{eventsDetails.description} </Text>
                          </CardItem>
                          <CardItem>
                              <Left>
                                  <Button transparent>
...
if(_this.state.resultData==true){
			...
			var resultData=<View style={{alignSelf:"center"}}>
				<Text style={{color: '#4B87C8' , fontWeight:'bold'}}>No Summary Avaliable..!</Text>		
			</View>;			
		}	
if(_this.state.retry==true){
 ...
 <Header style={{backgroundColor: '#FFFF45',}}>
	...
	<Text style={styles.titleViewText}>Learning Summary</Text>
	</View>
	        <ScrollView>
