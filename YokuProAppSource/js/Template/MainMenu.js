	...
	var url="http://54.255.203.182/yokupro/web/manage/";
		...
		.then((responseData) => {
			...
			"Response Body -> " + JSON.stringify(responseData)
		   		...
		   		_this.setState({
					mainMenuArray:responseData,visible:false,retry:true,logout:true
				});
		...
		.done();
				...
				this.setState({retry:true,isVisible:false,logout:true});
...
navigate(route){
  ...
  render() {
	...
	if(this.state.logout==true){
			...
			var logout=<Icon name='md-log-out' style={{color:'#ffffff',paddingTop:50,paddingLeft:20 }} onPress={() =>this.rowPressed('Login')} />;			
	}
	   return (
